...
class SearchCriteria:
    ...
    def _from_data(data: dict[str, str]) -> "SearchCriteria":
        ...
        return SearchCriteria(
            ...
            criteria_id=int(data[command.ATTR_SEARCH_CRITERIA_ID]),
            wildcard=data[command.ATTR_WILDCARD] == command.VALUE_YES,
            container_id=data.get(command.ATTR_CONTAINER_ID),
...
class SearchResult:
    ...
    def _from_message(message: HeosMessage, heos: "Heos") -> "SearchResult":
        ...
        return SearchResult(
            heos=heos,
            source_id=source_id,
            criteria_id=message.get_message_value_int(command.ATTR_SEARCH_CRITERIA_ID),
            search=message.get_message_value(command.ATTR_SEARCH),
...
class MultiSearchResult:
    ...
    def _from_message(message: HeosMessage, heos: "Heos") -> "MultiSearchResult":
        """Create a new instance from a message."""
        source_ids = message.get_message_value(command.ATTR_SOURCE_ID).split(",")
        criteria_ids = message.get_message_value(command.ATTR_SEARCH_CRITERIA_ID).split(
            ","
        )
        statisics = SearchStatistic._from_string(
            message.get_message_value(command.ATTR_STATS)
        )
        ...
        return MultiSearchResult(
            ...
            errors=SearchStatistic._from_string(
                message.get_message_value(command.ATTR_ERROR_NUMBER)
            ),
        )
