...
class HeosPlayer:
    ...
    async def refresh_state(self) -> None:
        ...
        assert self._heos, "Heos instance not set"
        self.state = await self._heos.player_get_play_state(self.player_id)

    async def refresh_now_playing_media(self) -> None:
        """Pull the latest now playing media."""
        assert self._heos, "Heos instance not set"
        await self._heos.get_now_playing_media(self.player_id, self.now_playing_media)

    async def refresh_volume(self) -> None:
        """Pull the latest volume."""
        assert self._heos, "Heos instance not set"
        self.volume = await self._heos.player_get_volume(self.player_id)

    async def refresh_mute(self) -> None:
        """Pull the latest mute status."""
        assert self._heos, "Heos instance not set"
        self.is_muted = await self._heos.player_get_mute(self.player_id)

    async def refresh_play_mode(self) -> None:
        """Pull the latest play mode."""
        assert self._heos, "Heos instance not set"
        play_mode = await self._heos.player_get_play_mode(self.player_id)
    ...
    async def set_state(self, state: const.PlayState) -> None:
        ...
        assert self._heos, "Heos instance not set"
        await self._heos.player_set_play_state(self.player_id, state)
    ...
    async def set_volume(self, level: int) -> None:
        ...
        assert self._heos, "Heos instance not set"
        await self._heos.player_set_volume(self.player_id, level)

    async def set_mute(self, state: bool) -> None:
        """Set the mute state."""
        assert self._heos, "Heos instance not set"
        await self._heos.player_set_mute(self.player_id, state)
    ...
    async def volume_up(self, step: int = const.DEFAULT_STEP) -> None:
        """Raise the volume."""
        assert self._heos, "Heos instance not set"
        await self._heos.player_volume_up(self.player_id, step)

    async def volume_down(self, step: int = const.DEFAULT_STEP) -> None:
        """Raise the volume."""
        assert self._heos, "Heos instance not set"
        await self._heos.player_volume_down(self.player_id, step)

    async def toggle_mute(self) -> None:
        """Toggle mute state."""
        assert self._heos, "Heos instance not set"
        await self._heos.player_toggle_mute(self.player_id)

    async def set_play_mode(self, repeat: const.RepeatType, shuffle: bool) -> None:
        """Set the play mode of the player."""
        assert self._heos, "Heos instance not set"
        await self._heos.player_set_play_mode(self.player_id, repeat, shuffle)

    async def clear_queue(self) -> None:
        """Clear the queue of the player."""
        assert self._heos, "Heos instance not set"
        await self._heos.player_clear_queue(self.player_id)

    async def play_next(self) -> None:
        """Clear the queue of the player."""
        assert self._heos, "Heos instance not set"
        await self._heos.player_play_next(self.player_id)

    async def play_previous(self) -> None:
        """Clear the queue of the player."""
        assert self._heos, "Heos instance not set"
        await self._heos.player_play_previous(self.player_id)
    ...
    async def play_input_source(
        ...
        assert self._heos, "Heos instance not set"
        await self._heos.play_input_source(self.player_id, input_name, source_player_id)

    async def play_preset_station(self, index: int) -> None:
        """Play the favorite by 1-based index."""
        assert self._heos, "Heos instance not set"
        await self._heos.play_preset_station(self.player_id, index)

    async def play_url(self, url: str) -> None:
        """Play the specified URL."""
        assert self._heos, "Heos instance not set"
        await self._heos.play_url(self.player_id, url)

    async def play_quick_select(self, quick_select_id: int) -> None:
        """Play the specified quick select."""
        assert self._heos, "Heos instance not set"
        await self._heos.player_play_quick_select(self.player_id, quick_select_id)
    ...
    async def play_media(
        ...
        """
        assert self._heos, "Heos instance not set"
        await self._heos.play_media(self.player_id, media, add_criteria)

    async def set_quick_select(self, quick_select_id: int) -> None:
        """Set the specified quick select to the current source."""
        assert self._heos, "Heos instance not set"
        await self._heos.player_set_quick_select(self.player_id, quick_select_id)

    async def get_quick_selects(self) -> dict[int, str]:
        """Get a list of quick selects."""
        assert self._heos, "Heos instance not set"
        return await self._heos.get_player_quick_selects(self.player_id)
